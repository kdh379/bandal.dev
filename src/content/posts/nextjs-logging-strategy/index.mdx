  ...
  if (typeof window !== "undefined") {
    ...
    window.addEventListener(
      ...
      (event) => {
        ...
        // Next.js 프레임워크가 "re-render" 방지를 위해 "NEXT_HTTP_ERROR_FALLBACK;404" 에러를 throw 하는 것을 무시
        const isNotFoundThrow = errorMessage
          .toString()
          .includes("NEXT_HTTP_ERROR_FALLBACK;404");
        if (isNotFoundThrow) {
          return;
        }
    ...
    window.onunhandledrejection = (event) => {
      ...
      // Next.js 프레임워크가 "re-render" 방지를 위해 "NEXT_REDIRECT" 에러를 throw 하는 것을 무시
      const isNextRedirectThrow = isRedirectError(error);
      if (isNextRedirectThrow) {
        return;
      }

...
import solidjs from "./solidjs.svg";
import jsframeworkbenchmark from "./js-framework-benchmark.png";

export const meta = {
  title: "SolidJS 사용기: React와의 닮아있다.",
  description:
    "웹개발은 React로만 하고 있었는데, SolidJS를 사용해보며 느낀 것들을 정리했습니다.",
  date: "2025-05-18",
};

<Image src={solidjs} alt="SolidJS" className="max-w-xs mx-auto" />
...
React의 디자인 철학과 일치한다고 하지만, 가상 DOM을 사용하지 않고 직접 DOM 업데이트를 수행한다. [js-framework-benchmark](https://krausest.github.io/js-framework-benchmark/2025/table_chrome_136.0.7103.93.html) 기준, React 대비 엄청난 성능차이를 보여준다. 거의 Vanilla JS 수준이다.

<Image src={jsframeworkbenchmark} alt="SolidJS" />
...
- https://www.solidjs.com/guides/comparison#react 에서 참고
  ...
  return (
    <div>
      {/* Show 컴포넌트는 when 조건이 참일 때 children을, 
          거짓일 때 fallback을 렌더링 */}
      <Show when={props.isLoggedIn} fallback={<p>로그인해주세요</p>}>
        <p>환영합니다!</p>
      </Show>
...
function TodoList(props) {
  return (
    <ul>
      <For each={props.todos}>{(todo) => <li>{todo.text}</li>}</For>
    </ul>
  );
}
```
...
SolidJS는 hydration 이후 클라이언트에서 코드가 다시 실행되지 않는다. 이는 Next.JS의 SSR과 다른 동작 방식으로, Next.JS는 hydration 이후 이벤트 주입을 위해 전체 코드를 재실행 하는 방식과 다르다.

만일 `window` 객체에 접근해야할 코드가 있다면, React에선 조건문을 통해 `if(typeof window !== 'undefined')` 를 쓰면 SSR에선 건너뛰고, hydration 과정에서 클라이언트에서 실행된다. 하지만 SolidJS에선 그렇게하면 window에 접근할 수 없다. 대신, `clientOnly` 함수를 통해 클라이언트전용 컴포넌트임을 명시하여 사용해야만 한다.
...
- [ ] Hydration 관련 문제 해결
  - Next.JSeh Hydration 디버깅이 어려운데, SolidJS는 문제가 된 Element 딱 한줄만 보여준다.
